...
// alternar concluída (SUBSTITUI o antigo alternarConcluida)
export async function concluir(id: number, userId: number): Promise<boolean> {
  const todas = await carregar();

  const tarefa = todas.find(t => t.id === id && t.userId === userId);
...
export async function remover(id: number, userId: number): Promise<boolean> {
  ...
  const index = todas.findIndex(t => t.id === id && t.userId === userId);
...
// export central (opcional, mas útil)
